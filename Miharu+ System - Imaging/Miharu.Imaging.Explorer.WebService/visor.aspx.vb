...
#Region " Metodos "
    ...
    Private Sub Config_Page()
        If Request.QueryString.Count > 0 Then
            ...
            TipoDB = Request.QueryString("tipoDB")
        Else
            Return
        End If

        Session("Token") = Token
        Session("TipoDB") = TipoDB

        Clear_Cache()
    End Sub

    Private Sub Load_Data()
        If (Session("Token") IsNot Nothing AndAlso Session("Token").ToString() <> "" And Session("TipoDB") IsNot Nothing AndAlso Session("TipoDB").ToString() <> "") Then
    ...
    Private Sub Mostrar()
        If (Token <> "") Then
            Try
                ...
                tvVisor.ImageUrl = DescargarImagenes(Token, TipoDB)
...
#Region " Funciones "
    ...
    'Agregar parametro
    Private Function DescargarImagenes(ByVal nToken As String, ByVal TipoDB As String) As List(Of String)
        ...
        Try
            ...
            Else
                dbmCore = New DBCore.DBCoreDataBaseManager(Program.CoreRisksConnectionString)
                'traza = "Crear imaging"
                dbmImaging = New DBImaging.DBImagingDataBaseManager(Program.ImagingRisksConnectionString)
            End If
            ...
            If (FileRow.Es_Anexo) Then
                ...
                '    manager = New FileProviderManager(FileRow.fk_Anexo, dbmImaging, 0, Program.Remoting)
                'Else
            ...
            Else
                ...
                For folio As Short = 1 To Folios
                    ...
                    Select FileRow.fk_Content_Type
                        ...
                        Case Else
                            FileName = TempPath & Guid.NewGuid().ToString() & ".tif"
                            ImageManager.Save(New FreeImageAPI.FreeImageBitmap(New MemoryStream(Image)), FileName, "", ImageManager.EnumFormat.Tiff, ImageManager.EnumCompression.Lzw, False, TempPath)
