...
Namespace DesktopReportDataGridView
    Public Class DesktopReportDataGridViewControlValidation
...
#Region " Metodos "
        ...
        Public Sub Exportar(nEncabezado As Boolean, nComma As Boolean, nTab As Boolean, nSemicolom As Boolean, nBlank As Boolean, Excel As Boolean, Texto As Boolean, Optional Salto_Linea As String = "")
                            ...
                            If Contadorshowmessage_ = 0 Then
                                If GuardarFileDialog.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
                                    If ResultadoReporteDesktopDataGridViewControl.RowCount > 0 Then
                                        ...
                                        DataGridViewToCSV(ResultadoReporteDesktopDataGridViewControl, fs, "", False, Salto_Linea)
                                        Archivo = "[" + fs.Name + "]"
                                        filename = fs.Name
                                        GuardaArchivo_ = filename.Replace(nombre, "")
                        ...
                        If ButtonOnClickReportMas_ Then
                            If Contadorshowmessage_ = 0 Then
                                If GuardarFileDialog.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
                                    ...
                                    If ResultadoReporteDesktopDataGridViewControl.RowCount > 0 Then
                                        Dim fs = CType(GuardarFileDialog.OpenFile(), FileStream)
                                        DataGridViewToCSV(ResultadoReporteDesktopDataGridViewControl, fs, sSeparador, nEncabezado, Salto_Linea)
                                        Archivo = "[" + fs.Name + "]"
                                        filename = fs.Name
                                        GuardaArchivo_ = filename.Replace(nombre, "")
                                    End If
        ...
        Public Shared Sub DataGridViewToCSV(ByRef data As DesktopDataGridViewControl, ByRef archivo As FileStream, Optional ByVal separator As String = ",", Optional ByVal Encabezado As Boolean = True, Optional ByVal SaltoLinea As String = "")
...
#Region " Eventos "
        ...
        Private Sub ExportarToolStripButton_Click(sender As System.Object, e As System.EventArgs) Handles ExportarToolStripButton.Click
            ...
            If ParametrosForm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                Exportar(ParametrosForm.ManejaEncabezado, ParametrosForm.SeparadoComa, ParametrosForm.SeparadoTabulador, ParametrosForm.SeparadoPuntoyComa, ParametrosForm.SeparadoVacio, ParametrosForm.FormatExcel, ParametrosForm.FormatTexto, Salto_linea_)
            End If
        End Sub
...
#End Region
        ...
        Private Sub GestionVencimientosToolStripButton_Click(sender As System.Object, e As System.EventArgs) Handles GestionVencimientosToolStripButton.Click
            Dim Respuesta = DesktopMessageBox.DesktopMessageBoxControl.DesktopMessageShow("¿Esta seguro que desea enviar los documentos al despacho", "Gestion de vencimientos", Desktop.Controls.DesktopMessageBox.DesktopMessageBoxControl.IconEnum.AdvertencyIcon, False)
            If (Respuesta = DialogResult.OK) Then
                Dim dbmArchiving As New DBArchiving.DBArchivingDataBaseManager(Me.ConnectionString_Archiving)

                Try
                    dbmArchiving.Connection_Open(1)
                    Dim solicitud = dbmArchiving.SchemaCustody.TBL_Solicitud.DBNextId
                    Dim dataTable = ResultadoReporteDesktopDataGridViewControl.DataSource

                    Dim SolicitudType As New DBArchiving.SchemaCustody.TBL_SolicitudType()
                    SolicitudType.id_Solicitud = solicitud
                    SolicitudType.fk_Usuario = Usuario
                    SolicitudType.fk_Usuario_Destino = Usuario
                    SolicitudType.fk_Solicitante = Nothing
                    SolicitudType.Fecha_Solicitud = Now()
                    SolicitudType.Bloqueada = False
                    SolicitudType.Fecha_Mail_Prestamo = Nothing
                    dbmArchiving.SchemaCustody.TBL_Solicitud.DBInsert(SolicitudType)
                    Dim Contador = 0
                    For Each row As DataRow In dataTable.Rows
                        Contador = Contador + 1
                        dbmArchiving.SchemaReport.PA_Ejecutar_Proximo_Vencer.DBExecute(CStr(row(3)), Contador, Entidad, Proyecto, Esquema, solicitud)
                    Next

                    DesktopMessageBox.DesktopMessageBoxControl.DesktopMessageShow("Documentos en gestión para envio al Cliente bajo el No. Solicitud " + solicitud.ToString, "Miharu Desktop", Desktop.Controls.DesktopMessageBox.DesktopMessageBoxControl.IconEnum.SuccessfullIcon, True)

                    For Each frm As Form In System.Windows.Forms.Application.OpenForms
                        If frm.Name = "FormProximosVencer" Then
                            frm.Close()
                        End If
                    Next
