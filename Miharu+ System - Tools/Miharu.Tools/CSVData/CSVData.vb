...
Namespace csv
    ...
    Public Class CSVData
        ...
        Dim mSeparator As Char = ","c
        Dim mTextQualifier As Char = """"c
        Dim mData() As String
        Dim mHeader As Boolean

        Private regQuote As New Regex("^(\x22)(.*)(\x22)(\s*,)(.*)$", CType(RegexOptions.IgnoreCase + RegexOptions.RightToLeft, RegexOptions))
        Private regNormal As New Regex("^([^,]*)(\s*,)(.*)$", CType(RegexOptions.IgnoreCase + RegexOptions.RightToLeft, RegexOptions))
...
#Region " Regular Expressions "
        ...
        Private Sub SetupRegEx()
            ...
            regQuote = New Regex(sQuote, CType(RegexOptions.IgnoreCase + RegexOptions.RightToLeft, RegexOptions))
            regNormal = New Regex(sNormal, CType(RegexOptions.IgnoreCase + RegexOptions.RightToLeft, RegexOptions))
            regQuoteLast = New Regex(sQuoteLast, CType(RegexOptions.IgnoreCase + RegexOptions.RightToLeft, RegexOptions))
            regNormalLast = New Regex(sNormalLast, CType(RegexOptions.IgnoreCase + RegexOptions.RightToLeft, RegexOptions))
        End Sub
            ...
            For Each dr In dsCSV.Tables("CSVData").Rows
                ...
                For iCol = 0 To dsCSV.Tables("CSVData").Columns.Count - 1
                    ...
                    If Not dr(iCol) Is DBNull.Value Then
                        If InStr(CStr(dr(iCol)), mSeparator) > 0 Then
                            sLine &= mTextQualifier & CStr(dr(iCol)) & mTextQualifier
                        Else
                            sLine &= CStr(dr(iCol))
                        End If
                    End If
