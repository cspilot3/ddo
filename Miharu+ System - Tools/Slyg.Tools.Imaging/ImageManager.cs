...
namespace Slyg.Tools.Imaging
{
    ...
    public class ImageManager
    {
        ...
        // Obtiene la imagen del pdf en el formato deseado con una resolución sin redimensionar
        public static byte[] GetUnresizedFolioDataPdfOnly(string nFileName, int nFolio, EnumFormat nFormat, EnumCompression nCompression, FreeImageAPI.FREE_IMAGE_SAVE_FLAGS nFlags)
        {
            return GetUnresizedFolioDataPdf(nFileName, nFolio, nFormat, nCompression, nFlags);
        }
        ...
        /// <summary>
        /// Extrae datos de un pdf por su folio utilizando la resolución original de la imagen.
        /// </summary>
        /// <param name="nFileName">El nombre de archivo del PDF.</param>
        /// <param name="nFolio">El número de folio del que se extraen los datos.</param>
        /// <param name="format">El formato de salida deseado (por ejemplo, TIFF, JPEG).</param>
        /// <param name="Compresion">El nivel de compresión que se aplicará.</param>
        /// <param name="flags">Banderas de guardado adicionales.</param>
        /// <returns>Una matriz de bytes que contiene los datos del folio sin redimensionar en el formato dado.</returns>
        private static byte[] GetUnresizedFolioDataPdf(string nFileName, int nFolio, EnumFormat format, EnumCompression Compresion, FreeImageAPI.FREE_IMAGE_SAVE_FLAGS flags)
        {
            using (var newImage = GetFolioBitmap(nFileName, nFolio))
            {
                // Se encarga de limitar el ancho de la iamgen, ya que la libreria puede traer una imagen con un ancho muy grande, por defecto se establece en 1800
                int imageWidthMax = 1800;

                if (newImage.Width > imageWidthMax)
                {
                    double resizeratio = imageWidthMax / (double)newImage.Size.Width;

                    // Redimenciona la imagen estableciendo como limite un ancho de imageWidthMax
                    var nSize = new System.Drawing.Size((int)(newImage.Size.Width * resizeratio), (int)(newImage.Size.Height * resizeratio));
                    newImage.Rescale(nSize, FreeImageAPI.FREE_IMAGE_FILTER.FILTER_BICUBIC);
                    newImage.SetResolution(200, 200);
                }                 
