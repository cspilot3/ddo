    ...
    public partial class FormExport : FormBase
    {
                ...
                if (LlenaExpedientesSeleccion())
                {
                    ...
                    if (Respuesta == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            //Se crea Tabla de FechaRecaudos
                            DataTable FechaRecaudos = new DataTable();
                            ...
                            if (FechaRecaudos.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow rowFechaRecaudos in FechaRecaudos.Rows)
                                {
                                    dynamic ServidoresDataTable = dbmImaging.SchemaProcess.PA_Exportacion_Servidor.DBExecute(Convert.ToInt32(rowFechaRecaudos[1]));
                                    dynamic FolderDataTable = new DBImaging.SchemaProcess.CTA_Exportacion_FoldersDataTable();
                                    var FileDataTable = new DBIntegration.SchemaBcoPopular.CTA_Exportacion_Expedientes_AsistidosDataTable();
                                    ...
                                    string FileFolderName = rowFechaRecaudos[0].ToString().Replace("/", "") + @"\";
                                    ...
                                    foreach (var RowServidor in ServidoresDataTable)
                                    {
                                        ...
                                        try
                                        {
                                            lock (BloqueoConcurrencia)
                                            {
                                                dynamic servidor = dbmImaging.SchemaProcess.PA_Exportacion_Servidor.DBExecute(Convert.ToInt32(rowFechaRecaudos[1]))[0].ToCTA_ServidorSimpleType();
                                            ...
                                            //var GruposD = (from g in FileDataTable.AsEnumerable() select g.Field<int>("fk_Grupo")).ToList();
                                            ...
                                            foreach (int grupo in GruposD)
                                            {
                                                ...
                                                if (grupo == 0)
                                                {
                                                    ...
                                                    foreach (DataRowView ItemFile in FilesbyGroupDataView)
                                                    {
                                                        ...
                                                        // Enviar el archivo
                                                        ArrayList ArrayParameters = new ArrayList();
                                                        ...
                                                        AgregarHilo(ArrayParameters);

                                                        foreach (DBIntegration.SchemaBcoPopular.CTA_Exportacion_Expedientes_AsistidosRow Item in FileDataTable)
                                                        {
                                                            if (Item.fk_Expediente == Convert.ToInt64(ItemFile["fk_Expediente"]) & Item.fk_Folder == Convert.ToInt64(ItemFile["fk_Folder"]) & Item.fk_File == (short)Convert.ToInt64(ItemFile["fk_File"]))
                                                            {
                                                                Item.Nombre_Imagen_File = ItemFile["Nombre_Imagen_File"].ToString();
                                                                break;
                                                            }
                                                        }
        ...
        private void ExportarImagen(FileProviderManager nManager, DataRowView ItemFile, ImageManager.EnumCompression nCompresion, string nFileFolderName)
        {
            try
            {
                ...
                lock (BloqueoConcurrencia)
                {
                    Folios = nManager.GetFolios(Convert.ToInt64(ItemFile["fk_Expediente"]), Convert.ToInt16(ItemFile["fk_Folder"]), Convert.ToInt16(ItemFile["fk_File"]), Convert.ToInt16(ItemFile["id_Version"]));
                    ...
                    for (short folio = 1; folio <= Folios; folio++)
                    {
                        ...
                        nManager.GetFolio(Convert.ToInt64(ItemFile["fk_Expediente"]), Convert.ToInt16(ItemFile["fk_Folder"]), Convert.ToInt16(ItemFile["fk_File"]), Convert.ToInt16(ItemFile["id_Version"]), (short)folio, ref Imagen, ref Thumbnail);
        ...
        private void GenerarVisor(DBCore.DBCoreDataBaseManager dbmCore, DBImaging.DBImagingDataBaseManager dbmImaging, int idOT, string OutputFolder, DBImaging.SchemaProcess.CTA_Exportacion_FoldersDataTable FolderDataTable, DBIntegration.SchemaBcoPopular.CTA_Exportacion_Expedientes_AsistidosDataTable FileDataTable, DBImaging.SchemaProcess.CTA_Exportacion_DataDataTable FileDataDataTable, DBImaging.SchemaProcess.CTA_Exportacion_ValidacionesDataTable FileValidacionesDataTable)
        {
        ...
        public void ProcesoHilosExportacionImagenes(object objectArray)
        {
            ArrayList ArraListParameters = (ArrayList)objectArray;

            FileProviderManager manager = (FileProviderManager)ArraListParameters[0];
            DataRowView ItemFile = (DataRowView)ArraListParameters[1];
            ImageManager.EnumCompression nCompresion = (ImageManager.EnumCompression)ArraListParameters[2];
            string nFileFolderName = (string)ArraListParameters[3];
            DBIntegration.SchemaBcoPopular.CTA_Exportacion_Expedientes_AsistidosDataTable FileDataTable = (DBIntegration.SchemaBcoPopular.CTA_Exportacion_Expedientes_AsistidosDataTable)ArraListParameters[4];

            try
            {
                ExportarImagen(manager, ItemFile, nCompresion, nFileFolderName);
            }
            catch (Exception ex)
            {
        ...
        Object BloqueoConcurrencia = new object();

        public void AgregarHilo(ArrayList ArrayParameters)
        {
            while (TieneHiloslibres() == false)
            {
                Thread.Sleep(1000);
            ...
            Thread Threads = new Thread(ProcesoHilosExportacionImagenes);
        ...
        public bool TieneHiloslibres()
        {
            lock(BloqueoConcurrencia)
            {
                ...
                {
                    if (hilo.ThreadState.Equals(System.Diagnostics.ThreadState.Terminated))
                    {
        ...
        private DataTable getExpedientes()
        {
            DBImaging.DBImagingDataBaseManager dbmImaging = null;
            ...
            try
            {
                ...
                return dbmIntegration.SchemaBcoPopular.PA_Exportacion_Expediente_Asistidos.DBExecute(this.FechaProcesoDateTimePicker.Value.ToString("yyyy/MM/dd"), this.FechaProcesoFinalDateTimePicker.Value.ToString("yyyy/MM/dd"), this._Plugin.Manager.ImagingGlobal.Entidad, this._Plugin.Manager.ImagingGlobal.Proyecto);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, Program.AssemblyTitle, MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public FormExport(AsistidosPlugin _plugin)
        {
            InitializeComponent();
