...
Imports DBSecurity
Imports Miharu.CargueImagenes.Servicio
...
#Region " Eventos "
        ...
        Private Sub ProbarButton_MouseDown(ByVal sender As System.Object, ByVal e As MouseEventArgs) Handles ProbarButton.MouseDown
            ...
            ElseIf (e.Button = MouseButtons.Right) Then
                Dim obj As New CargueImagenesService
                obj.IniciarServicio()
            End If
        End Sub
...
#Region " Metodos "
        ...
        Private Sub LoadConfig()
            ...
            Try
                ...
                If (File.Exists(Program.AppDataPath + CargueImagenesConfig.ConfigFileName)) Then
            ...
            PasswordTextBox.Text = CargueImagenesConfig.Decrypt(Program.Config.Password)
            LogFolderTextBox.Text = Program.Config.RutaLog
            LogActivoCheckBox.Checked = Program.Config.LogActivo
        ...
        Private Sub SaveServico()
            If Validar() Then
                ...
                Program.Config.Password = CargueImagenesConfig.Encrypt(PasswordTextBox.Text)
                Program.Config.RutaLog = LogFolderTextBox.Text
                Program.Config.LogActivo = LogActivoCheckBox.Checked

                CargueImagenesConfig.Serialize(Program.Config, Program.AppDataPath)
        ...
        Private Sub ActivarGuardar()
            If Not _Cargando Then
                ...
                HayCambios = (Program.Config.User <> UserTextBox.Text) Or
                             ...
                             (CargueImagenesConfig.Decrypt(Program.Config.Password) <> PasswordTextBox.Text) Or
                             (Program.Config.RutaLog <> LogFolderTextBox.Text) Or
                             (Program.Config.LogActivo <> LogActivoCheckBox.Checked) Or
        ...
        Private Sub MonitorearServicio()
            ...
            Try
                ...
                For Each ServicioEncontrado As ServiceController In Servicios
                    If ServicioEncontrado.ServiceName = "CargueImagenesService" Then
                ...
                Else
                    ...
                    Select Case _Servicio.Status
                        Case ServiceControllerStatus.Running
                            ...
                            NotificacionNotifyIcon.Text = "MiharuCargueImagenesService [Activo]"

                        Case Else
                            EstadoStatusStripLabel.Text = "Servicio detenido"
                            NotificacionNotifyIcon.Icon = _NotifiIcon_Inactivo
                            NotificacionNotifyIcon.Text = "MiharuCargueImagenesService [Inactivo]"
        ...
        Private Sub IniciarServicio()
            Try
                ...
                NotificacionNotifyIcon.BalloonTipTitle = "MiharuCargueImagenesDDOService"
                NotificacionNotifyIcon.BalloonTipText = "El servicio MiharuCargueImagenesService ha sido activado"
        ...
        Private Sub DetenerServicio()
            Try
                If _Servicio.CanStop Then
                    ...
                    NotificacionNotifyIcon.BalloonTipTitle = "MiharuCargueImagenesService"
                    NotificacionNotifyIcon.BalloonTipText = "El servicio MiharuCargueImagenesService ha sido desactivado"
                    NotificacionNotifyIcon.ShowBalloonTip(5000)
                End If
            Catch ex As Exception
        ...
        Public Sub ValidarServicio()
            ...
            Try
                Dim ConnectionStrings As CargueImagenesConfig.TypeConnectionString
                ...
                ConnectionStrings = CargueImagenesConfig.getCadenasConexion(WebService)
...
#Region " Funciones "
        ...
        Private Function Validar() As Boolean
            ...
            End If
            If Program.Config.LogActivo = True Then
                If LogFolderTextBox.Text = "" Then
                    MessageBox.Show("Debe ingresar la ruta de la carpeta para el log", "Datos inválidos", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    tcBase.SelectedIndex = 1
                    LogFolderTextBox.Focus()
                    Resp = False
                ElseIf Not Directory.Exists(LogFolderTextBox.Text) Then
                    MessageBox.Show("La ruta de la carpeta para el log no es válida", "Datos inválidos", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    tcBase.SelectedIndex = 1
                    LogFolderTextBox.Focus()
                    Resp = False
                End If
        ...
        Private Sub tpServicio_Click(sender As Object, e As EventArgs) Handles tpServicio.Click

        End Sub

        Private Sub LogFolderTextBox_TextChanged(ByVal sender As System.Object, ByVal e As EventArgs) Handles LogFolderTextBox.TextChanged

            ActivarGuardar()
        End Sub

        Private Sub LogActivoCheckBox_CheckStateChanged(sender As Object, e As EventArgs) Handles LogActivoCheckBox.CheckStateChanged
            ActivarGuardar()
        End Sub
