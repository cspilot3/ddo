...
Public Class CargueImagenesConfig
#Region " Estructuras "
...
#Region " Declaraciones "
    ...
    Public Const ConfigFileName As String = "CargueImagenesDDOConfig.dat"
...
#Region " Serializar "
    ...
    Public Shared Sub Serialize(ByRef nObjectConfig As CargueImagenesConfig, ByVal nPath As String)
        If (Not Directory.Exists(nPath)) Then Directory.CreateDirectory(nPath)

        Dim Serializador As New XmlSerializer(GetType(CargueImagenesConfig))
    ...
    Public Shared Function Deserialize(ByVal nPath As String) As CargueImagenesConfig
        Dim Serializador = New XmlSerializer(GetType(CargueImagenesConfig))
        Dim Lector = New StreamReader(nPath.TrimEnd("\"c) & "\" & ConfigFileName)

        Try
            Return CType(Serializador.Deserialize(Lector), CargueImagenesConfig)
        Catch ex As Exception
            Throw ex
        Finally
            Lector.Close()
