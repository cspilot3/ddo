...
namespace  WebSantander.site.account
{
    public partial class connect : page_site
    {
        ...
        public override void Config_Page()
        {
            ...
            var Sesion = WebService.GetAppSession(Program.idModulo, this.MiharuSession.Usuario.id);

            if (Sesion != null && (Sesion.Client_IP != this.MiharuSession.ClientIPAddress || Sesion.Activo) &&
        ...
        private void GoToMain()
        {
            ...
            var WebService = new Miharu.Security.Library.WebService.SecurityWebService(Program.SecurityWebServiceURL, this.MiharuSession.ClientIPAddress);
            this.SessionToken = WebService.RegisterAppSession(Program.idModulo, this.MiharuSession.Usuario.id);
