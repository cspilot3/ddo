...
Public Class DescargaMasivaImagenesConfig
...
#Region " Estructuras "
    ...
    Public Structure TypeConnectionString
        ...
        Public DescargaMasiva As String
...
#Region " Propiedades "
    ...
    Public Property RutaPath As String
...
#Region " Serializar "
    ...
    Public Shared Sub Serialize(ByRef nObjectConfig As DescargaMasivaImagenesConfig, ByVal nPath As String)
        If (Not Directory.Exists(nPath)) Then Directory.CreateDirectory(nPath)

        Dim Serializador As New XmlSerializer(GetType(DescargaMasivaImagenesConfig))
    ...
    Public Shared Function Deserialize(ByVal nPath As String) As DescargaMasivaImagenesConfig
        Dim Serializador = New XmlSerializer(GetType(DescargaMasivaImagenesConfig))
        Dim Lector = New StreamReader(nPath.TrimEnd("\"c) & "\" & ConfigFileName)

        Try
            Return CType(Serializador.Deserialize(Lector), DescargaMasivaImagenesConfig)
        Catch ex As Exception
            Throw ex
        Finally
            Lector.Close()
    ...
    Public Shared Function getCadenasConexion(ByRef nWebService As SecurityWebService) As TypeConnectionString
        ...
        For Each Modulo As TypeModulo In ConnectionStrings
            Select Case CType(Modulo.Id, Program.Modulo)
                ...
                Case Program.Modulo.DescargaMasiva : Cadenas.DescargaMasiva = Modulo.ConnectionString
                Case Program.Modulo.Integration : Cadenas.Integration = Modulo.ConnectionString
            End Select
        Next
