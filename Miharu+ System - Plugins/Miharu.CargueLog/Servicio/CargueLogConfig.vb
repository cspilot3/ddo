...
#Region " Declaraciones "
    ...
    Public Const ConfigFileName As String = "CargueLogDDOConfig.dat"
...
#Region " Serializar "
    ...
    Public Shared Sub Serialize(ByRef nObjectConfig As CargueLogConfig, ByVal nPath As String)
        If (Not Directory.Exists(nPath)) Then Directory.CreateDirectory(nPath)

        Dim Serializador As New XmlSerializer(GetType(CargueLogConfig))
    ...
    Public Shared Function Deserialize(ByVal nPath As String) As CargueLogConfig
        Dim Serializador = New XmlSerializer(GetType(CargueLogConfig))
        Dim Lector = New StreamReader(nPath.TrimEnd("\"c) & "\" & ConfigFileName)

        Try
            Return CType(Serializador.Deserialize(Lector), CargueLogConfig)
        Catch ex As Exception
            Throw ex
        Finally
            Lector.Close()
