...
Public Class SynergeticsConfig
...
#Region " Declaraciones "
    ...
    Public Const ConfigFileName As String = "SynergeticsConfig.dat"
...
#Region " Serializar "
    ...
    Public Shared Sub Serialize(ByRef nObjectConfig As SynergeticsConfig, ByVal nPath As String)
        If (Not Directory.Exists(nPath)) Then Directory.CreateDirectory(nPath)

        Dim Serializador As New XmlSerializer(GetType(SynergeticsConfig))
    ...
    Public Shared Function Deserialize(ByVal nPath As String) As SynergeticsConfig
        Dim Serializador = New XmlSerializer(GetType(SynergeticsConfig))
        Dim Lector = New StreamReader(nPath.TrimEnd("\"c) & "\" & ConfigFileName)

        Try
            Return CType(Serializador.Deserialize(Lector), SynergeticsConfig)
        Catch ex As Exception
            Throw ex
        Finally
            Lector.Close()
