...
namespace  Miharu.Client.Browser.site.account
{
    public partial class login : page_site
    {
        ...
        private void Validate_Locked_IP()
        {
            try
            {
                this.SessionManager.ClientIPAddress = Program.getIPName();                
                var WebService = new Security.Library.WebService.SecurityWebService(Program.SecurityWebServiceURL, this.SessionManager.ClientIPAddress);

                if (WebService.IsIPBloqueada())
                    throw new Exception("La dirección IP: " + this.SessionManager.ClientIPAddress + " se encuentra bloqueada por exceder el número de intentos de conexión fallidos, por favor comuníquese con el administrador del sistema");
            }
            catch (Exception ex)
            {
        ...
        private void IniciarSesion(string nUserName, string nPassword)
        {
            ...
            {                
                ...
                try
                {
                    ...
                    {
                        var LocalSession = this.SessionManager;
                        ...
                        switch (LogonResult)
                        {
                            case EnumValidateUser.CAMBIAR_PASSWORD:
                                if (this.SessionManager.Usuario.PerfilManager.Permisos.Count > 0)
                                {
                                    LocalSession.Usuario.Password = nPassword;

                                    this.SessionManager.Parameter[consts.BackPage] = Navigation.site.account.login;
