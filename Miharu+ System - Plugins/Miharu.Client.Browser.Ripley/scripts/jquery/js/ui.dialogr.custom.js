...
////    var qHtml = $("<div id='LockBackground' style='position: absolute;z-index: 15000;width: 100%;height: 100%;top: 0;left: 0;background-color: #FFFFFF;filter: alpha(opacity=65);-moz-opacity: 0.65;opacity: 0.65;'></div>");
////    $('body').append(qHtml);
//}
...
//    window.alert = function (mess, callback) { Mensaje(mess, callback); };
//});

function DeferFocus(id) {
    setTimeout(function () { DeferFocusCallBack(id); }, 100);
}

function DeferFocusCallBack(id) {
    try {
        document.getElementById(id).focus();
    } catch (e) { }
}

function CerrarVentana(id, result) {
    try {
        var ctr = document.getElementById(id);
        $("#" + id).remove();
        if (ctr != null && ctr.ConfirmacionCallBack != undefined && ctr.ConfirmacionCallBack != null) ctr.ConfirmacionCallBack(result);
    } catch (e) { }
...
var alertSecuence = 0;
function Mensaje(msg, callBack) {
    var id = "VentanaAlert" + alertSecuence;
    var tid = "AlertText" + alertSecuence;
    var bid = "AlertButton" + alertSecuence++;
    var max = msg.length;
    var maxlines = (max / 46) + 1;
    var h = 10 + (maxlines * 15);
    h = (h > 80) ? h : 80;
    h = (h > 300) ? 300 : h;

    var wh = (h + 100);
    //border:1px Solid Gray;
    var html = "<div id='" + id + "'><div><table width='330px' style='text-align:left;'>" +
                        "<tr><td colspan='2'><div id='" + tid + "' style='height:" + h + "px; overflow:auto'></div></td></tr>" +
                        "<tr><td colspan='2' align='right'><input id='" + bid + "' type='button' value='Aceptar' onclick='return CerrarVentana(\"" + id + "\",true);' /></td></tr>" +
                    "</table></div></div>";
    var qHtml = $(html);
    $('body').append(qHtml);
    $('#' + tid).html(msg.toString());
    DeferFocus(bid);
    document.getElementById(id).ConfirmacionCallBack = callBack;

    $("#" + id).dialogr({ autoOpen: true, resizable: false, title: 'Mensaje', maximized: false, minimized: false, width: 360, height: wh, close: function (c) { CerrarVentana(c.target.id, false); } });
    return true;
}

var confirmSecuence = 0;
function Confirmacion(msg, callBack) {
    var id = "Ventanaconfirm" + confirmSecuence;
    var tid = "confirmText" + confirmSecuence;
    var bid = "confirmButton" + alertSecuence++;
    var max = msg.length;
    var maxlines = (max / 46) + 1;
    maxlines += SubStringCount(msg.toUpperCase(), "<BR/>");
    ...
    var html = "<div id='" + id + "'><div><table width='330px' style='text-align:left;'>" +
                        "<tr><td colspan='2'><div id='" + tid + "' style='height:" + h + "px; overflow:auto'></div></td></tr>" +
                        "<tr><td colspan='2' align='right'><table><tr><td><input id='" + bid + "' type='button' value='Cancelar' onclick='return CerrarVentana(\"" + id + "\",false);' /></td><td><input type='button' value='Aceptar' onclick='return CerrarVentana(\"" + id + "\",true);' /></td></tr></table></td></tr>" +
                    "</table></div></div>";
    var qHtml = $(html);
    $('body').append(qHtml);
    $('#' + tid).html(msg);
    ...
    $("#" + id).dialogr({ autoOpen: true, resizable: false, title: 'Confirmacion', maximized: false, minimized: false, width: 360, height: wh, close: function (c) { CerrarVentana(c.target.id, false); } });
    return false;
}

var Ventanas_DivDlg = [];
function ShowDivDialog(id, t, w, h, ok, cancel, fnOK, fnCancel) {
    var html = null;
    for (var i = 0; i < Ventanas_DivDlg.length; i++) {
        if (Ventanas_DivDlg[i][0] == id) {
            html = Ventanas_DivDlg[i][1];
        }
    }
    if (html == null) {
        html = "<div id='" + id + "'>" + $("#" + id).html() + "</div>";
        $("#" + id).remove();
        Ventanas_DivDlg[Ventanas_DivDlg.length] = [id, html];
    }

    $('body').append(html);
    $("#" + id).dialogr(
    { autoOpen: true, resizable: false, title: t,
        maximized: false, minimized: false, width: w, height: h,
        close: function (c) { CloseDivDialog(id); }
    });

    $('[id]', "#" + id).each(function (i, l) { try { Global.CreateID(l.id); } catch (e) { } }); //Parche para MIHARU

    setTimeout(function () {
        if (cancel != "" && cancel != undefined && cancel != null) {
            $("#" + cancel).click(function () {
                if (fnCancel != undefined) fnCancel();
                CloseDivDialog(id);
            });
        }

        if (ok != "" && ok != undefined && ok != null) {
            $("#" + ok).click(function () {
                if (fnOK == undefined) CloseDivDialog(id);
                else {
                    if (fnOK()) {
                        CloseDivDialog(id);
                    }
