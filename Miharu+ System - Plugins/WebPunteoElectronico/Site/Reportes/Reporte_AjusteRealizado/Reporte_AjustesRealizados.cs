...
namespace WebPunteoElectronico.Site.Reportes.Reporte_AjusteRealizado
{
    public class Reporte_AjustesRealizados : WebReport
    {
        #region Declaraciones

        private int IdUsuario;
        private const string Path_Nodo = "4.7";
        ...
        public override void Launch(ref ReportViewer nReportViewer, Dictionary<string, object> nParameters)
        {
            ...
            try
            {
                ...
                DataTable DatosDataTable = dbmBanagrario.SchemaReport.PA_Ajustes_Realizados.DBExecute(IdRegional,
                                                                                                     idCOB,
                                                                                                     IdOficina,
                ...
                var reportParameters = new List<ReportParameter> {new ReportParameter("UsuarioLogin", Login), new ReportParameter("Regional", NombreRegional), new ReportParameter("Cob", NombreCOB), new ReportParameter("Oficina", NombreOficina), new ReportParameter("Url", Program.VisorTxDetalle)};

                // Si se pasan los parámetros como null o "", tampoco aparece el reporte...

                var reportDataSource = new ReportDataSource("CTA_Reporte_AjustesRealizados", DatosDataTable);

                nReportViewer.LocalReport.DataSources.Clear();
                nReportViewer.LocalReport.ReportPath = Reports.Site.Reportes.Reporte_AjusteRealizado.Reporte_AjustesRealizados;
