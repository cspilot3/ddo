...
{
    public partial class WebReportViewerG : FormBase
    {
        #region Propiedades
        ...
        protected override void Config_Page()
        {
            ...
            this.FechaInicialTextBox.Text = DateTime.Now.AddDays(-1).ToString("yyyy/MM/dd").Replace('-','/');
            this.FechaFinalTextBox.Text = this.FechaInicialTextBox.Text;
            ...
            try
            {
                ...
                var UsuariosDataTable = DBMWorkflow.SchemaSecurity.CTA_Usuario_Banagrario.DBGet(0, new DBWorkFlow.SchemaSecurity.CTA_Usuario_BanagrarioEnumList(DBWorkFlow.SchemaSecurity.CTA_Usuario_BanagrarioEnum.Apellidos_Usuario, true));

                this.UsuarioFindDropDownList.Items.Clear();
                this.UsuarioFindDropDownList.Items.Add(new ListItem("- Todos - ", "-1"));

                foreach (var Fila in UsuariosDataTable)
                {
                    if (Fila.Apellidos_Usuario != null)
                        this.UsuarioFindDropDownList.Items.Add(new ListItem(Fila.Apellidos_Usuario.Trim() + " " + Fila.Nombres_Usuario.Trim(), Fila.Login_Usuario));
                }
            ...
            var report = this.WebReport;
            if (report != null)
                this.RegionalFindDropDownList.SelectedValue = report.getParameter("idRegional");
            Cargar_COBs(short.Parse(this.RegionalFindDropDownList.SelectedValue));
        }
        ...
        {
            if (!Slyg.Tools.DataConvert.IsDate(this.FechaInicialTextBox.Text, Slyg.Tools.DataConvert.EnumDateFormat.yyyyMMdd, '/'))
            {
                this.Master.ShowAlert("La fecha de movimiento inicial debe tener un formáto válido", MsgBoxIcon.IconWarning);
            }
            else if (!Slyg.Tools.DataConvert.IsDate(this.FechaFinalTextBox.Text, Slyg.Tools.DataConvert.EnumDateFormat.yyyyMMdd, '/'))
            {
                this.Master.ShowAlert("La fecha de movimiento final debe tener un formáto válido", MsgBoxIcon.IconWarning);
            }            
            else if (Slyg.Tools.DataConvert.ToDate(this.FechaInicialTextBox.Text, Slyg.Tools.DataConvert.EnumDateFormat.yyyyMMdd, '/').Value > Slyg.Tools.DataConvert.ToDate(this.FechaFinalTextBox.Text, Slyg.Tools.DataConvert.EnumDateFormat.yyyyMMdd, '/').Value)
            {
            ...
            else
            {
                try
                {
                    var Parametros = new Dictionary<string, object> {{"ConnectionString", this.ConnectionString}, {"Usuario", this.UsuarioFindDropDownList.SelectedItem.Text}, {"IdRadicacion", this.RadicacionFindTextBox.Text}, {"IdRegional", short.Parse(this.RegionalFindDropDownList.SelectedValue)}, {"NombreRegional", this.RegionalFindDropDownList.SelectedItem.Text}, {"IdCOB", short.Parse(this.COBFindDropDownList.SelectedValue)}, {"NombreCOB", this.COBFindDropDownList.SelectedItem.Text}, {"IdOficina", int.Parse(this.OficinaFindDropDownList.SelectedValue)}, {"NombreOficina", this.OficinaFindDropDownList.SelectedItem.Text}, {"FechaInicial", this.FechaInicialTextBox.Text}, {"FechaFinal", this.FechaFinalTextBox.Text}, {"Login", this.MiharuSession.Usuario.Login}, {"MiharuSesion", this.MiharuSession}};
