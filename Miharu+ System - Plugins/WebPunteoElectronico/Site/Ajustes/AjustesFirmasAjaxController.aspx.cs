...
{
    public partial class AjustesFirmasAjaxController : FormBase
    {
        ...
        private void GuardarObservacion()
        {
            ...
            try
            {
                ...
                var observacion = new DBAgrario.SchemaFirmas.TBL_Ajuste_ObservacionType();

                switch (Accion)
                {
                    case "Nueva":
                        observacion.id_Ajuste_Observacion = dbmBanagrario.SchemaFirmas.TBL_Ajuste_Observacion.DBNextId();
                        observacion.Observacion_Ajuste = Obs;
                        observacion.Activa = true;
                        dbmBanagrario.SchemaFirmas.TBL_Ajuste_Observacion.DBInsert(observacion);
                        Cod = observacion.id_Ajuste_Observacion.ToString();
                        break;
                    case "Editar":
                        observacion.Observacion_Ajuste = Obs;
                        observacion.Activa = true;
                        dbmBanagrario.SchemaFirmas.TBL_Ajuste_Observacion.DBUpdate(observacion, Convert.ToInt32(Cod));
                        break;
                    default:
