...
//    $("#FechaInicial_Input").mask("9999/99/99");
//    $("#FechaInicial_Input").datepicker();

//    $("#FechaFinal_Input").mask("9999/99/99");
//    $("#FechaFinal_Input").datepicker();
...
function addFormData() {
    ...
    dt[dt.length] = { name: 'FechaInicialMov_Input', value: Get('FechaInicialMov_Input').value };
    dt[dt.length] = { name: 'FechaFinalMov_Input', value: Get('FechaFinalMov_Input').value };
    ...
    dt[dt.length] = { name: 'Usuarios_Input', value: Get('Usuarios_Input').value };
    dt[dt.length] = { name: 'Nro_Producto_Input', value: Get('Nro_Producto_Input').value };
    dt[dt.length] = { name: 'Nro_Ente_Input', value: Get('Nro_Ente_Input').value };
...
function MostrarCampos(val) {
    if (UltimoTipoTransaccion != val) {
        if (ArrayContains(TiposTransaccionList, val)) {
            ...
            $.ajax({
                type: "POST",
                url: "Consulta_Firmas_Data.aspx/MostrarCampos",
...
function MostrarOficinas(val) {
    $.ajax({
        type: "POST",
        url: "Consulta_Firmas.aspx/MostrarOficinas",
        data: "{'nStrCOB':'" + val + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
...
function Validar() {
    try {
        var validar = 0;
        if (EsVacio("FechaInicialMov_Input") == false) {
            if (EsVacio("FechaFinalMov_Input") == true) {
                validar++;
            }
        }
        else {
            validar++;
            LimpiarCampo("FechaFinalMov_Input");
        }
        if (EsVacio("FechaInicialProc_Input") == false) { 
            if (EsVacio("FechaFinalProc_Input") == true) {
            validar++;
           }
        }
        else{
            validar++;
            LimpiarCampo("FechaFinalProc_Input");
        }

        if (validar == 2) throw "Se debe escoger por lo menos un rango de fechas";

//        EsVacio("COB_Input", "COB");
//        EsVacio("FechaInicial_Input", "Fecha inicial");
//        EsVacio("FechaFinal_Input", "Fecha final");

        var difference = GetFecha("FechaFinalMov_Input", "Fecha inicial") - GetFecha("FechaInicialMov_Input", "Fecha inicial");
        ...
        var differenceProc = GetFecha("FechaFinalProc_Input", "Fecha inicial") - GetFecha("FechaInicialProc_Input", "Fecha inicial");
        var daysProc = Math.round(differenceProc / (1000 * 60 * 60 * 24));

        if (daysProc < 0) {throw "La fecha final debe ser mayor a la fecha inicial"; }
        if (daysProc >= 15) {throw "El rango de fechas debe ser maximo de 15 dias"; }
