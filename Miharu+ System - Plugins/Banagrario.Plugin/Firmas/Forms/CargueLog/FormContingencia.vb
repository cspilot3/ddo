...
#Region " Eventos "
        ...
        Private Sub ExcluirButton_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles ExcluirButton.Click
            ExcluirTransaccionesContingencia()
        End Sub
...
#Region " Metodos "
        ...
        Private Sub ExcluirTransaccionesContingencia()
            Dim FechaProceso = FechaProcesoPicker.Value.ToString("yyyyMMdd")
            If Validar() Then
                If MsgBox("Está seguro de excluir la información del cruce contra el log ?", MsgBoxStyle.YesNo, "Excluidos Contingencia") = MsgBoxResult.Yes Then
                    ...
                    Try
                        ...
                        Dim data = dbmAgrario.SchemaFirmas.PA_ExcluirFirmasContingencia.DBExecute(FechaProceso, OficinaDesktopComboBox.SelectedValue.ToString, TransaccionesDesktopComboBox.SelectedValue.ToString, NumeroCuentaTextbox.Text.Trim)
                        If data.Rows.Count > 0 Then
                            Dim Excluidos As String
                            Excluidos = Trim(data.Rows(0).Item("RegistrosActualizados").ToString())
                            If Not Excluidos = "0" Then
                                DesktopMessageBoxControl.DesktopMessageShow("Se realizó el proceso correctamente, se excluyeron " & Excluidos & " transacciones por Contingencia.", "Excluidos del Cruce por Contingencia", DesktopMessageBoxControl.IconEnum.SuccessfullIcon, True)
                            Else
                    ...
                    Catch ex As Exception
                        ...
                        DesktopMessageBoxControl.DesktopMessageShow("Excluir Transacciones por Contingencia", ex)
                    Finally
...
#Region " Funciones "
        ...
        Private Function Validar() As Boolean
            ...
            If NumeroCuentaTextbox.Text.Trim = "" Then
                DesktopMessageBoxControl.DesktopMessageShow("Debe seleccionar un Numero de Cuenta", "Excuidos Contingencia", DesktopMessageBoxControl.IconEnum.WarningIcon, True)
                FechaProcesoPicker.Focus()
                Return False
            End If

            If FechaProcesoPicker.Value.ToString = "" Then
                DesktopMessageBoxControl.DesktopMessageShow("Debe seleccionar una fecha de Movimiento", "Excuidos Contingencia", DesktopMessageBoxControl.IconEnum.WarningIcon, True)
                FechaProcesoPicker.Focus()
                Return False
            End If
