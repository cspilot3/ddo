...
#Region " Declaraciones "
    ...
    Public Const ConfigFileName As String = "ReportesConfig.dat"
...
#Region " Serializar "
    ...
    Public Shared Sub Serialize(ByRef nObjectConfig As ReportesConfig, ByVal nPath As String)
        If (Not Directory.Exists(nPath)) Then Directory.CreateDirectory(nPath)

        Dim Serializador As New XmlSerializer(GetType(ReportesConfig))
    ...
    Public Shared Function Deserialize(ByVal nPath As String) As ReportesConfig
        Dim Serializador = New XmlSerializer(GetType(ReportesConfig))
        Dim Lector = New StreamReader(nPath.TrimEnd("\"c) & "\" & ConfigFileName)

        Try
            Return CType(Serializador.Deserialize(Lector), ReportesConfig)
        Catch ex As Exception
            Throw ex
        Finally
            Lector.Close()
