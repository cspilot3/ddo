...
#Region " Declaraciones "
    ...
    Public Const ConfigFileName As String = "SFTPTransferConfig.dat"
...
#Region " Serializar "
    ...
    Public Shared Sub Serialize(ByRef nObjectConfig As SFTPTransferConfig, ByVal nPath As String)
        If (Not Directory.Exists(nPath)) Then Directory.CreateDirectory(nPath)

        Dim Serializador As New XmlSerializer(GetType(SFTPTransferConfig))
    ...
    Public Shared Function Deserialize(ByVal nPath As String) As SFTPTransferConfig
        Dim Serializador = New XmlSerializer(GetType(SFTPTransferConfig))
        Dim Lector = New StreamReader(nPath.TrimEnd("\"c) & "\" & ConfigFileName)

        Try
            Return CType(Serializador.Deserialize(Lector), SFTPTransferConfig)
        Catch ex As Exception
            Throw ex
        Finally
            Lector.Close()
