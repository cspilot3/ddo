    ...
    public partial class FormExportar : Form
    {
        ...
        private void ExportarButton_Click(object sender, EventArgs e)
        {
            if (validar())
            {
                ...
                try
                {
                    ...
                    FileDataTable = dbmIntegration.SchemaBcoDavivienda.PA_GetExpedientesExportacionEmbargos.DBExecute(this._Plugin.Manager.ImagingGlobal.Entidad, this._Plugin.Manager.ImagingGlobal.Proyecto, Convert.ToInt32(FechaProcesoDateTimePicker.Value.ToString("yyyyMMdd")), Convert.ToInt32(FechaProcesoFinalDateTimePicker.Value.ToString("yyyyMMdd")), Convert.ToInt32(OTDesktopComboBox.SelectedValue == "--TODOS--" ? -1 : OTDesktopComboBox.SelectedValue));
                    FileDataDataTable = dbmIntegration.SchemaBcoDavivienda.PA_GetExpedientesDataExportacionEmbargos.DBExecute(this._Plugin.Manager.ImagingGlobal.Entidad, this._Plugin.Manager.ImagingGlobal.Proyecto, Convert.ToInt32(FechaProcesoDateTimePicker.Value.ToString("yyyyMMdd")), Convert.ToInt32(FechaProcesoFinalDateTimePicker.Value.ToString("yyyyMMdd")), Convert.ToInt32(OTDesktopComboBox.SelectedValue == "--TODOS--" ? -1 : OTDesktopComboBox.SelectedValue));

                    int cantfilesxcarpeta = 0;

                    cantfilesxcarpeta = Convert.ToInt32(dbmIntegration.SchemaConfig.TBL_Parametro_Sistema.DBFindByfk_Entidadfk_ProyectoNombre_Parametro_Sistema(this._Plugin.Manager.ImagingGlobal.Entidad, this._Plugin.Manager.ImagingGlobal.Proyecto, "CantidadFilesxCarpetaEmbargos")[0].Valor_Parametro_Sistema);
                        ...
                        if (Respuesta == DialogResult.Yes)
                        {
                            ...
                            foreach (string sigla in esquemaExportacion)
                            {
                                ...
                                if (cantidadCarpetas == 1)
                                {
                                    ...
                                    //crear el archivo
                                    var sw = new StreamWriter(File.Open(OutputFolderCarpeta + "\\" + NomArchivo + ".csv", FileMode.Create), new System.Text.UTF8Encoding(false));
                                    sw.Close();
                                    sw.Dispose();
                                    ...
                                    foreach (DataRowView ItemFile in FilesbyGroupDataView)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            sw = new StreamWriter(File.Open(OutputFolderCarpeta + "\\" + NomArchivo + ".csv", FileMode.Append), new System.Text.UTF8Encoding(false));
                                ...
                                else //cantidad de carpetas > 1
                                {
                                    ...
                                    foreach (DataRowView ItemFile in FilesbyGroupDataView)
                                    {
                                        if (crearnuevacarpeta)
                                        {
                                            ...
                                            //crear el archivo
                                            sw = new StreamWriter(File.Open(OutputFolderCarpeta + "\\" + NomArchivo + ".csv", FileMode.Create), new System.Text.UTF8Encoding(false));
                                            sw.Close();
                                            sw.Dispose();
                                        }
        ...
        private void ExportarImagen(FileProviderManager nManager, DataRowView ItemFile, ImageManager.EnumCompression nCompresion, string nFileFolderName)
        {
            try
            {
                ...
                Folios = nManager.GetFolios(Convert.ToInt64(ItemFile["fk_Expediente"]), Convert.ToInt16(ItemFile["fk_Folder"]), Convert.ToInt16(ItemFile["fk_File"]), 1);
                ...
                for (short folio = 1; folio <= Folios; folio++)
                {
                    ...
                    {
                        nManager.GetFolio(Convert.ToInt64(ItemFile["fk_Expediente"]), Convert.ToInt16(ItemFile["fk_Folder"]), Convert.ToInt16(ItemFile["fk_File"]), 1, (short)folio, ref Imagen, ref Thumbnail);
                    }                   
        ...
        private void CargarOT()
        {
            ...
            try
            {
                ...
                var OTDataTable = dbmImaging.SchemaProcess.PA_Get_OT_Rango_Fecha.DBExecute(_Plugin.Manager.ImagingGlobal.ProyectoImagingRow.fk_Entidad, _Plugin.Manager.ImagingGlobal.ProyectoImagingRow.fk_Proyecto, System.Convert.ToInt32(FechaProcesoDateTimePicker.Value.ToString("yyyyMMdd")), System.Convert.ToInt32(FechaProcesoFinalDateTimePicker.Value.ToString("yyyyMMdd")), false);

                if (OTDataTable.Count > 0)
                {
                    foreach (var OTRow in OTDataTable)
                        Utilities.LlenarCombo(ref OTDesktopComboBox, OTDataTable, OTDataTable.id_OTColumn.ColumnName, OTDataTable.id_OTColumn.ColumnName, true, "-1", "--TODOS--");
                }
            }
            catch (Exception ex)
            {
        ...
        private void FechaProcesoDateTimePicker_ValueChanged(object sender, EventArgs e)
        {
            CargarOT();
        }

        private void FechaProcesoFinalDateTimePicker_ValueChanged(object sender, EventArgs e)
        {
            CargarOT();
        }
    }
