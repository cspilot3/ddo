Imports Santander.Plugin.EmbargosDesembargos
Imports DBImaging
...
Namespace EmbargosDesembargos.Forms
    Public Class FormGenerarCartas
        Private _plugin As ImagingPlugin

        Public Sub New(nPlugin As ImagingPlugin)
            _plugin = nPlugin
        ...
        Private Sub GenerarCartas()
            ...
            Try
                If Validar() Then
                    ...
                    Dim ExpedientesDataTable = dbmIntegration.SchemaSantander.PA_Get_Expedientes_Carta_Respueta.DBExecute(CInt(OTDesktopComboBox.SelectedValue), _plugin.Manager.ImagingGlobal.Entidad, _plugin.Manager.ImagingGlobal.Proyecto)
                    Dim NoClientesDataTable = dbmIntegration.SchemaSantander.PA_Get_No_ClientesCR.DBExecute(System.Convert.ToInt32(OTDesktopComboBox.SelectedValue), _plugin.Manager.ImagingGlobal.Entidad, _plugin.Manager.ImagingGlobal.Proyecto)
                    ...
                    If ExpedientesDataTable.Rows.Count > 0 Then
                        ...
                        ProgressForm.SetProceso("Generar Cartas")
                        ProgressForm.SetAccion("Obteniendo imágenes...")
                        ProgressForm.SetProgreso(0)
                        ProgressForm.SetMaxValue(ExpedientesDataTable.Rows.Count)
                        ...
                        For Each ExpedientesRow In ExpedientesDataTable
                            ...
                            For Each tblFormatoRow As TBL_FormatoRow In FormatoDataTable
                                If CInt(ExpedientesRow.TipoCartaRespuesta) = tblFormatoRow.Id_Formato Then

                                    Dim CartaFormateadaRow = tblFormatoRow.ToTBL_FormatoSimpleType

                                    'Formateo de Partes
                                    Dim FormatoParametrosDataTable = dbmIntegration.SchemaProcess.PA_Formato_Parametros.DBExecute(ExpedientesRow.fk_Expediente, ExpedientesRow.fk_Folder, 1, CInt(ExpedientesRow.TipoCartaRespuesta))
                                    ...
                                    'Dim FechaGeneracion As String = DateTime.Now.ToString("D", cultures)

                                    CartaFormateadaRow.Destinatario = CartaFormateadaRow.Destinatario.Replace("@FechaProceso", FechaProcesoPicker.Value.ToString("dd 'de' MMMM 'de' yyyy"))
                                    ...
                                    Dim NoClientesDT As DataTable = NoClientesDataTable.Select("fk_Expediente = " + ExpedientesRow.fk_Expediente.ToString()).CopyToDataTable()
                                    ...
                                    Dim NoClientesDataSource = New ReportDataSource("CTA_No_ClientesCRDataSet", NoClientesDT)
                                    ...
                                    ReportViewer1.LocalReport.ReportEmbeddedResource = "Santander.Plugin.Report_GenerarCartasED.rdlc"
                                    ...
                                    ReportViewer1.LocalReport.DataSources.Add(NoClientesDataSource)
                                    ...
                                    FileName = Program.AppPath & Program.TempPath & identificador.ToString() & ".tif"
                                    ...
                                    Dim Folios As Integer = ImageManager.GetFolios(FileName)
                                    ...
                                    For folio As Short = 1 To Folios
                                        Dim dataImage = ImageManager.GetFolioData(FileName, folio, formato, compresion)
