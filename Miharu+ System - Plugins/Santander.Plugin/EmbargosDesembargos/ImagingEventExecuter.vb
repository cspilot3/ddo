...
#Region " Implementacion IEventExecuter "
        ...
        Public Sub FinalizarCruceGenerico(nidEntidad As Short, nidProyecto As Short, nidFechaProceso As Integer, nidOt As Integer) Implements Miharu.Imaging.Library.Eventos.IEventExecuter.FinalizarCruceGenerico
            ...
            Try
                dbmIntegration = New DBIntegration.DBIntegrationDataBaseManager(_plugin.SantanderConnectionString)
                dbmIntegration.Connection_Open(_plugin.Manager.Sesion.Usuario.id)

                dbmIntegration.SchemaSantander.PA_Insercion_Data_Documento_Respuesta.DBExecute(nidEntidad, nidProyecto, nidFechaProceso, nidOt, _plugin.Manager.Sesion.Usuario.id)
            Catch ex As Exception
                MessageBox.Show("Error : " & ex.Message, "Finalizar Cruce Generico", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Finally
        ...
        Public Sub FinalizarPrepararDataGenerico(nidEntidad As Short, nidProyecto As Short, nidFechaProceso As Integer, nidOt As Integer) Implements Miharu.Imaging.Library.Eventos.IEventExecuter.FinalizarPrepararDataGenerico
            Dim dbmIntegration As DBIntegration.DBIntegrationDataBaseManager = Nothing
            Try
                dbmIntegration = New DBIntegration.DBIntegrationDataBaseManager(_plugin.SantanderConnectionString)
                dbmIntegration.Connection_Open(_plugin.Manager.Sesion.Usuario.id)

                dbmIntegration.SchemaSantander.PA_Finalizar_Preparar_Data.DBExecute(nidEntidad, nidProyecto, nidFechaProceso, nidOt)
            Catch ex As Exception
                MessageBox.Show("Error : " & ex.Message, "Finalizar Preparar Data Generico", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Finally
                If (dbmIntegration IsNot Nothing) Then dbmIntegration.Connection_Close()
            End Try
        End Sub
