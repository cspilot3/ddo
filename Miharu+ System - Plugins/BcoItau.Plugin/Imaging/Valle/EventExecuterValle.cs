...
{
    public class EventExecuterValle : EventExecuter, IEventExecuter
    {
        ...
        public EventExecuterValle(VallePlugin nPlugin)
        {
        ...
        public void ValidarSavePrimeraCaptura(System.Collections.Generic.List<Miharu.Imaging.Indexer.View.Indexacion.CampoCaptura> campos, int fk_documento, ref bool Result)
        {
            ...
            try {
	            dbmIntegration = new DBIntegration.DBIntegrationDataBaseManager(_Plugin.BcoItauConnectionString);
                ...
                if ((drowCampoFiltrado_aux.Count > 0))
                {
                    ...
                    {
                        var dtResult = dbmIntegration.SchemaBcoItau.TBL_Formularios_Unicos.DBFindByFormulario((FORMULARIO.Trim()));
                        ...
                        {
                            DBIntegration.SchemaBcoItau.TBL_Formularios_UnicosType rowInsertar = new DBIntegration.SchemaBcoItau.TBL_Formularios_UnicosType();
                            rowInsertar.Formulario = FORMULARIO;
                            dbmIntegration.SchemaBcoItau.TBL_Formularios_Unicos.DBInsert(rowInsertar);
                            Result = true;
                        }
                    }
                ...
                if (FechaProcesoDataTable.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (drowCampoFiltrado_aux_FechaRecaudo.Count > 0)
                        {
                            ...
                            var dtFechaRecaudo = dbmIntegration.SchemaBcoItau.PA_Obtiene_Valor_FileData.DBExecute(nCampo.fk_Expediente, fk_Documento, idCampoFechaRecaudo);
                            ...
                            if (nCampo.fk_Campo == id_Campo)
                            {
                                ...
                                if (!string.IsNullOrEmpty((FORMULARIO.Trim())))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        var EncontradoUpdate = dbmIntegration.SchemaBcoItau.TBL_Formularios_Unicos.DBFindByFormulario(nCampo.Valor_File_Data.ToString());

                                        if (EncontradoUpdate.Rows.Count > 0)
                                        {
                                            DBIntegration.SchemaBcoItau.TBL_Formularios_UnicosType rowUpdate = new DBIntegration.SchemaBcoItau.TBL_Formularios_UnicosType();
                                            ...
                                            dbmIntegration.SchemaBcoItau.TBL_Formularios_Unicos.DBUpdate(rowUpdate, idUpdate);
                                        }
                                        else
                                        {
                        ...
                        if (dbmIntegration.SchemaBcoItau.PA_Devolver_Cruce.DBExecute(Fecha_Proceso.ToString("yyyy/MM/dd"), Fecha_Recaudo).Rows[0]["RESULT"].ToString() == "OK")
                        {
