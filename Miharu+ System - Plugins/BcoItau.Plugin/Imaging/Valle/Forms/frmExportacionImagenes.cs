...
namespace BcoItau.Plugin.Imaging.Atlantico.Forms
{
    public partial class frmExportacionImagenes : Form
    {
        ...
        public frmExportacionImagenes(VallePlugin _plugin, string TipoReporte_IMG = "")
        {
            InitializeComponent();
            ...
            if(ValidaFechas(this.dtpFechaProcesoInicial.Value, this.dtFechaProcesoFinal.Value) == "")
            {
                ...
                try
                {
                    ...
                    while (dtInicial.Date <= dtFinal.Date)
                    {
                        ...
                        {
                            RegistrosExportar = dbmIntegration.SchemaBcoItau.PA_Obtiene_Datos_Exportar.DBExecute(dtInicial.ToString("yyyy/MM/dd"), dtInicial.ToString("yyyy/MM/dd"));
                        }
                        else if (this._tipoReporte_IMG == "INCONSISTENCIAS")
                        {
                            RegistrosExportar = dbmIntegration.SchemaBcoItau.PA_Imagenes_Inconsistencias.DBExecute(dtInicial.ToString("yyyy/MM/dd"), dtInicial.ToString("yyyy/MM/dd"));
                        }
                    ...
                    if (this._tipoReporte_IMG == "INCONSISTENCIAS")
                    {
                        Inconsistencias = dbmIntegration.SchemaBcoItau.PA_Reporte_Inconsistencias.DBExecute();
                        var Formato_Salida = dbmIntegration.SchemaBcoItau.TBL_Config_Reporte.DBFindByNombre_Reporte("INCONSISTENCIAS").FirstOrDefault();
        ...
        private void ActualizaImagen_Str(string Formulario, long fk_Expediente, int fk_folder, int fk_file, string imagenStr, DBIntegration.DBIntegrationDataBaseManager dbmIntegration)
        {
            try
            {
                dbmIntegration.SchemaBcoItau.PA_Actualiza_Imagen_Inconsistencias.DBExecute(Formulario, fk_Expediente, fk_folder, fk_file, imagenStr);
            }
            catch (Exception ex)
            {
        ...
        private bool ValidaFaltantes()
        {
            ...
            try
            {
                ...
                var FechasFaltantes = dbmIntegration.SchemaBcoItau.TBL_Registro_Faltantes.DBGet(null);
