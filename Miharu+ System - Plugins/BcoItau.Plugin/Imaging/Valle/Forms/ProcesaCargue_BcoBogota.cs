...
namespace BcoItau.Plugin.Imaging.Valle.Forms
{
    ...
    {
        public bool ProcesaLog_BcoBogota(ref DataTable data, string fileName, ref BackgroundWorker CargueBackgroundWorker, VallePlugin _Plugin, ref int IdCargue, DBIntegration.SchemaBcoItau.TBL_Log_DataType Data_Log, bool Valida_Duplicados, string NombreProceso, string FechaProceso, ref DesktopConfig.TypeResult retorno_result)
        {
            var dbmIntegration = new DBIntegrationDataBaseManager(_Plugin.BcoItauConnectionString);
            bool retorno = false;
            ...
            try
            {
                ...
                lock ((this))
                {
                    ...
                    TablaErrores = dbmIntegration.SchemaBcoItau.PA_Procesa_Log_Dinamico.DBExecute(Data_Log.fk_Tipo_log, Entidad, Proyecto, tabla, usuario, fileName, Data_Log.Fecha_Proceso);
