...
namespace BcoItau.Plugin.Imaging.Atlantico.Forms
{
    public partial class frmRelacionCuentasMunicipio : Form
    {
        ...
        #region Constructor
        public frmRelacionCuentasMunicipio(VallePlugin Plugin)
        {
            this._Plugin = Plugin;
            InitializeComponent();
        }
                ...
                else if (e.ColumnIndex == this.dgvCuentasMunicipios.Columns["Eliminar"].Index)
                {
                    if (DesktopMessageBoxControl.DesktopMessageShow("¿Realmente Desea Eliminar la Cuenta?", "Eliminar Cuenta", DesktopMessageBoxControl.IconEnum.WarningIcon, false) == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        try
                        {
                            ...
                            var regUpdate = new DBIntegration.SchemaBcoItau.TBL_Relacion_Cuentas_MunicipioType();
                            int idVigente = Convert.ToInt32(dbmIntegration.SchemaBcoItau.TBL_Estado_Cuenta.DBFindByNombre_Estado("Inactivo").ToList().First().id);
                            regUpdate.fk_Estado_Cuenta =  idVigente;

                            dbmIntegration.SchemaBcoItau.TBL_Relacion_Cuentas_Municipio.DBUpdate(regUpdate, idBuscar);
        ...
        private void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            {
                dbmIntegration.Connection_Open(_Plugin.Manager.Sesion.Usuario.id);
                DataTable ReportCuentas = dbmIntegration.SchemaBcoItau.PA_Reporte_Relaciones_Cuentas_Municipios.DBExecute();
        ...
        private void cargarGrilla(int? id = null, string No_Cuenta_Bco_Bogota = null, string nombre_cuenta = null, string Codigo_Compensación = null, string Nombre_Municipio = null, string Estado = null)
        {
            ...
            {
                dbmIntegration.Connection_Open(_Plugin.Manager.Sesion.Usuario.id);
                DataTable DtCuentas = dbmIntegration.SchemaBcoItau.PA_Select_Relacion_Cuentas_Municipios.DBExecute(id, No_Cuenta_Bco_Bogota, nombre_cuenta, Codigo_Compensación, Nombre_Municipio, Estado);
