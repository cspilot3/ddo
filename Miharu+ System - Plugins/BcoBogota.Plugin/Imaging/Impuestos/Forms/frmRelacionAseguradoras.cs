...
namespace BcoBogota.Plugin.Imaging.Impuestos.Forms
{
    public partial class frmRelacionAseguradoras : Form
    {
        ...
        public frmRelacionAseguradoras(ImpuestosPlugin Plugin)
        {
            this._Plugin = Plugin;
            InitializeComponent();
        }
                ...
                else if (e.ColumnIndex == this.dgvAseguradoras.Columns["Eliminar"].Index)
                {
                    if (DesktopMessageBoxControl.DesktopMessageShow("¿Realmente Desea Eliminar la Cuenta?", "Eliminar Cuenta", DesktopMessageBoxControl.IconEnum.WarningIcon, false) == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        try
                        {
                            ...
                            var regUpdate = new DBIntegration.SchemaBcoBogota.TBL_AseguradorasType();
                            regUpdate.Activo = false;

                            dbmIntegration.SchemaBcoBogota.TBL_Aseguradoras.DBUpdate(regUpdate, idBuscar);
        ...
        private void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            {
                dbmIntegration.Connection_Open(_Plugin.Manager.Sesion.Usuario.id);
                DataTable ReportCuentas = dbmIntegration.SchemaBcoBogota.PA_SelectStatement_Aseguradoras.DBExecute(null, null, null, null, null, null, false);
        ...
        private void cargarGrilla(int? id_Aseguradora = null, string Tipo = null,  string Nombre_Aseguradora = null, string Nit = null, string Codigo = null, string Dv = null)
        {
            ...
            {
                dbmIntegration.Connection_Open(_Plugin.Manager.Sesion.Usuario.id);
                DataTable DtCuentas = dbmIntegration.SchemaBcoBogota.PA_SelectStatement_Aseguradoras.DBExecute(id_Aseguradora, Tipo,  Nombre_Aseguradora, Nit, Codigo, Dv, true);
