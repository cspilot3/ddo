...
namespace BcoFalabella.Plugin.Imaging.Embargos
{
    public class EventExecuterEmbargos : EventExecuter, IEventExecuter
    {
        ...
        {
            DBIntegration.DBIntegrationDataBaseManager dbmIntegration = new DBIntegration.DBIntegrationDataBaseManager(this._Plugin.BcoFalabellaConnectionString);
            try
            {
                dbmIntegration.Connection_Open(this._Plugin.Manager.Sesion.Usuario.id);
                dbmIntegration.Transaction_Begin();
                dbmIntegration.SchemaBcoFalabella.PA_Finalizar_Preparar_Data.DBExecute(nidEntidad, nidProyecto, nidFechaProceso, nidOt);
