...
Namespace Forms.Empaque
    ...
    Public Class FormEmpaqueCancelados
        Inherits FormBase
...
#Region " Metodos "
        ...
        Private Sub cargaConfiguracionesIniciales()
            ...
            For Each row As DataRow In Cajas.Rows
                Dim conteo = dmcore.SchemaReport.PA_Ejecutar_Cancelados_Informe_Conteo.DBExecute(CType(Convert.ToInt64(row.Item("fk_expediente")), Global.Slyg.Tools.SlygNullable(Of Integer)))
                Folders += conteo.Rows.Count
                For i As Integer = 0 To conteo.Rows.Count - 1
                    TotalFolios += Convert.ToInt32(conteo.Rows(i).Item("Folios_File"))
                Next
            Next row
            ...
            DocumentosLabel.Text = Folders.ToString()
            FoliosLabel.Text = TotalFolios.ToString()
