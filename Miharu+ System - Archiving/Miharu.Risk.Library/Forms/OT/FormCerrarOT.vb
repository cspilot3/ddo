...
#Region " Métodos "
        ...
        Public Sub LlenarOTS()
            ...
            dbmArchiving.Connection_Open(Program.Sesion.Usuario.id)
            TableOT = dbmArchiving.SchemaRisk.TBL_OT.DBFindByfk_Estadofk_Entidadfk_Proyectofk_Sede_Procesamiento(DBCore.EstadoEnum.Creado, Program.RiskGlobal.Entidad, Program.RiskGlobal.Proyecto, Program.DesktopGlobal.CentroProcesamientoRow.fk_Sede)
            ...
            Else
                If DesktopMessageBoxControl.DesktopMessageShow("Esta seguro de cerrar la OT " & CStr(OTDataGridView.SelectedRows(0).Cells(SchemaRisk.TBL_OTEnum.id_OT.ColumnName).Value) & "?", "Cerrar OT", DesktopMessageBoxControl.IconEnum.AdvertencyIcon) = DialogResult.OK Then
                    ...
                    Try
                        dbmArchiving.Connection_Open(Program.Sesion.Usuario.id)
                        dbmArchiving.Transaction_Begin()
                        dbmArchiving.Schemadbo.PA_Cerrar_OT.DBExecute(CShort(OTDataGridView.SelectedRows(0).Cells(SchemaRisk.TBL_OTEnum.id_OT.ColumnName).Value), Program.Sesion.Usuario.id, DesktopConfig.Modulo.Archiving, DBCore.EstadoEnum.Cerrado)
                        dbmArchiving.Transaction_Commit()
                    Catch ex As Exception
                        dbmArchiving.Transaction_Rollback()
                    Finally
...
#Region " Eventos "
        ...
        Private Sub FormCerrarOt_Load(ByVal sender As System.Object, ByVal e As EventArgs) Handles MyBase.Load
            LlenarOTS()
        End Sub
