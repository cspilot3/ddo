...
using Slyg.Tools;
using Miharu.Imaging.WebService;
    ...
    public partial class Consulta : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var tblEntidad = dbmCore.SchemaSecurity.CTA_Rol_Entidades.DBFindByfk_Usuario(Utils.MySession.Usuario.id);
                ...
                PDFURL.Value = Utils.VisorURL.Replace("visor", "VisorPdf");
        ...
        [WebMethod]
        public static string Buscar(string par1, string val1, string idEntidad, string idProyecto, string idProyectoLlave, string nLlave, bool ntipoBusqueda)
        {
            ...
            try
            {
                ...
                SlygNullable<int> tipoBusqueda = DBNull.Value; //1-Llaves; 2-normal, 3-dataAsociada
                ...
                var info = dbmCore.SchemaProcess.PA_Busqueda_Get_Explorer_2.DBExecute(idCampoBusqueda, fkCampoTipo, valor, fkEntidad, fkProyecto, fkProyectoLlave, valorLlave, Utils.MySession.Usuario.id, tipoBusqueda);
                var cadena = (from DataRow data in info.Rows
                              select "{id_Expediente:'" + data["id_Expediente"].val() + "'" +
                                     ...
                                     ", Data_1: '" + data["Data_1"].val() + "'" +
                                     ", Data_2: '" + data["Data_2"].val() + "'" +
                                     ", Data_3: '" + data["Data_3"].val() + "'" +
                                     ", id_Entidad: '" + data["id_Entidad"].val() + "'" +
                                     ", FilesCustodia: '" + data["FilesCustodia"].val() + "'" +
                                     ", llaves: '" + data["llaves"].val() + "'" +
                                     ", Tipo: '" + data["Tipo"].val() + "'" +
                                     ", TipoDB:'" + data["TipoDB"].val() + "'" +
                                     ", FechaProceso:'" + data["FechaProceso"].val() + "'" +
                                     ", historia: ''" +
        ...
        [WebMethod]
        public static string GetFiles(string codigo, string llaves, string tipo, int tipoDB)
        {
            Utils.IsValidSession();
            ...
            try
            {
                ...
                var info = dbmCore.SchemaProcess.PA_Folder_File_Busqueda_Imaging_F11.DBExecute(codigo, Utils.MySession.Usuario.id, llaves, tipo, tipoDB);
                var resultados = (from DataRow data in info.Rows
                    select "{CBarras_File:'" + data["CBarras_File"].val() + "'" +
                           ...
                           ", Pdf: '" + data["Imaging"].val() + "'" +
                           ", File_Unique_Identifier: '" + data["File_Unique_Identifier"].val() + "'" +
                           ", Fk_Log_Cargue: '" + data["Fk_Log_Cargue"].val() + "'" +
                           ", fk_Documento: '" + data["fk_Documento"].val() + "'" +
                           ", fk_Expediente: '" + data["fk_Expediente"].val() + "'" +
                           ", fk_Folder: '" + data["fk_Folder"].val() + "'" +
                           ", id_File: '" + data["id_File"].val() + "'" +
                           ", TipoDB: '" + data["TipoDB"].val() + "'" +
                           ", Nombre_Documento: '" + data["Nombre_Documento"].val() + "'" +
                           ", historia: ''" +
        ...
        [WebMethod]
        public static string GetFileData(string codigo, int tipo, string fk_log_Cargue, string fk_Documento, string llaves, int fk_Expediente, int fk_Folder, int id_File, int TipoDB)
        {
            Utils.IsValidSession();
            DBCore.DBCoreDataBaseManager dbmCore = null;

            SlygNullable<int> fk_log_Cargue_ = null;
            SlygNullable<int> fk_Documento_ = null;

            if (fk_log_Cargue != "")
            {
                fk_log_Cargue_ = int.Parse(fk_log_Cargue);
            }

            if (fk_Documento != "")
            {
                fk_Documento_ = int.Parse(fk_Documento);
            }
            ...
            try
            {
                ...
                var files = dbmCore.Schemadbo.PA_File_Data_Busqueda_Imaging.DBExecute(codigo, Utils.MySession.Usuario.id, tipo, fk_log_Cargue_, llaves, fk_Documento_, fk_Expediente, fk_Folder, id_File,TipoDB);
                //var files = dbmCore.Schemadbo.PA_File_Data_Busqueda_F11.DBExecute(codigo, Utils.MySession.Usuario.id, tipo);
        ...
        [WebMethod]
        public static string GetFileDataAsociada(short fk_Documento, short fk_Campo, int fk_Expediente, short fk_Folder, short id_File, int TipoDB)
        {
            Utils.IsValidSession();
            DBCore.DBCoreDataBaseManager dbmCore = null;
            ...
            try
            {
                ...
                var DefinicionDataTable = dbmCore.SchemaConfig.TBL_Tabla_Asociada.DBFindByfk_Documentofk_CampoEliminado_Campo(fk_Documento, fk_Campo, false);
                ...
                foreach (var DefinicionRow in DefinicionDataTable)
                {
                    DataColumn NewColumna = new DataColumn();
                    NewColumna.Caption = DefinicionRow.Nombre_Campo;
                    NewColumna.ColumnName = DefinicionRow.Nombre_Campo.Replace(' ','_');

                    Data.Columns.Add(NewColumna);
                }

                var ValoresDataTable = dbmCore.SchemaProcess.PA_File_Data_Asociada_Campos_Activos.DBExecute(fk_Documento, fk_Expediente, fk_Folder, id_File, fk_Campo);

                var FileDataAsociadaView = new DataView(ValoresDataTable);
                var RegistrosAsociada = FileDataAsociadaView.ToTable(true, "fk_Expediente", "fk_Folder", "fk_File", "fk_Documento", "fk_Campo", "id_File_Data_Asociada");

                foreach (DataRow ItemRow in RegistrosAsociada.Rows)
                {

                    FileDataAsociadaView.RowFilter = "id_File_Data_Asociada = " + ItemRow["id_File_Data_Asociada"].ToString();
                    FileDataAsociadaView.Sort = "fk_Campo_Tabla";
                    var NewRow = Data.NewRow();
                    NewRow[0] = ItemRow["id_File_Data_Asociada"];

                    for (int Col = 0; Col <= FileDataAsociadaView.Count - 1; Col++)
                    {
                        if (Col <= DefinicionDataTable.Count)
                        {
                            NewRow[Col + 1] = FileDataAsociadaView[Col].Row["Valor_File_Data"];
                        }
                    }
        ...
        //[WebMethod]
        //public static string VerPDF(string token)
        //{
                ...
                {   
                    ...
                    else
                    {
                        ...
                        if (FileDataTable.Rows.Count > 0)
                        {
                            ...
                            {
                                var ExpedienteLlaves = dbmCore.SchemaProcess.CTA_Expediente_LLave.DBFindByid_Expediente(FileRow.fk_Expediente);
        ...
        [WebMethod]
        public static string GetHistorialSolicitud(string codigo, int TipoDB, int tipo)
        {
            Utils.IsValidSession();
            ...
            try
            {
                ...
                var infoImaging = new DBArchiving.Schemadbo.CTA_Consulta_Historial_SolicitudDataTable();
                var infoRisks = new DBArchiving.Schemadbo.CTA_Consulta_Historial_Solicitud_RisksDataTable();
                string Temp="";
                ...
                if (tipo == 1)
                {
                    infoImaging = dbmArchiving.Schemadbo.CTA_Consulta_Historial_Solicitud.DBFindByCBarras_FileCBarras_Folder(null, codigo);
                    Temp = infoImaging.Serialize();
                }   
                else if (tipo == 2) 
                {
                    if (TipoDB == 1) /*IMAGING*/
                    {
                        infoImaging = dbmArchiving.Schemadbo.CTA_Consulta_Historial_Solicitud.DBFindByCBarras_FileCBarras_Folder(codigo, null);
                        Temp = infoImaging.Serialize(); 
                    }
                    else if (TipoDB == 2) /*RISKS*/
                    {
                        infoRisks = dbmArchiving.Schemadbo.CTA_Consulta_Historial_Solicitud_Risks.DBFindByCBarras_FileCBarras_Folder(codigo, null);
                        Temp = infoRisks.Serialize();
                    }
