var PaginaCobroJuridico = "Reporte_CobroJuridico.aspx/";
...
function fillRepCobroJuridico(data) {
    ...
    if (data.length > 0) {
        ...
        $("#tblReporte").jqGrid({
            datatype: "local",
            colNames: ['Tipologia', 'Estado', 'No Obligacion', 'Id Deudor', 'Agencia', 'Codigo Garantia', 'Codigo Oficina', 'Codigo Linea', 'Fecha Solicitud', 'Motivo Solicitud', 'Usuario Solicitante', 'Usuario Destino'],
            colModel: [{ name: 'Tipologia', index: 'Tipologia', width: "100px" },
                        { name: 'Estado', index: 'Estado', width: '150px' },
                        { name: 'No_Obligacion', index: 'No_Obligacion', width: '100px' },
                        { name: 'Id_Deudor', index: 'Id_Deudor', width: '100px' },
                        { name: 'Agencia', index: 'Agencia', width: '100px' },                                 
                        ...
                        { name: 'Fecha_Solicitud', index: 'Fecha_Solicitud', width: '150px' },
                        { name: 'Motivo_Solicitud', index: 'Motivo_Solicitud', width: '150px' },
                        { name: 'Usuario_Solicitante', index: 'Usuario_Solicitante', width: '150px' },
                        { name: 'Usuario_Destino', index: 'Usuario_Destino', width: '150px'}],
            rowNum: 10,
            autowidth: true,
            height: '300px',
            rowList: [10, 20, 30],
            pager: '#divReporte',
            caption: "Reporte Cobro Juridico"
        });
        ...
        jAlert("No se encontraron datos", "Reporte Cobro Juridico");
        clearTable();
...
function buscarRepCobroJuridico() {
    if (Validar()) {
...
function Exportar() {
    try {
        ...
        ExportJQGridDataToExcel("#tblReporte", "CobroJuridico" + fecha + ".xlsx");
    }
    catch (ex) {
        alert(ex);
        return false;
