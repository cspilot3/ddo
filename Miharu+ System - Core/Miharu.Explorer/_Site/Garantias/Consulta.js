...
function fillFile(data) {
    ...
    $("#tblDocumentos").jqGrid({
        datatype: "local",
        colNames: ['S', 'HE', 'HS', 'D', 'I', 'P', 'Codigo', 'Folios', 'Monto', 'Tipologia', 'id_Estado', 'Estado', 'id_Entidad', 'File_Unique_Identifier', 'Fk_Log_Cargue', 'fk_Documento'],
        colModel: [{ name: 'Solicitado', index: 'Solicitado', width: '25px', formatter: solicitudFormat },
        ...
        onCellSelect: function (rowid, index) {
            ...
            switch (index) {
                case 0:
                    ...
                    break;
                case 1:
                    getHistorialEstados(codigo, fk_log_cargue, fk_Documento);
                    break;
                case 2:
                    GetHistorialSolicitud(codigo);
                    break;
                case 3:
                    GetFileData(codigo, fk_log_cargue, fk_Documento);
                    break;
                case 4:
...
function getFile(codigo, llaves, tipo) {
    $.ajax({
        dataType: "json"
        , contentType: content
        , type: "POST"
        , url: Pagina + "GetFiles"
        , data: "{codigo:'" + codigo + "' , llaves: '" + llaves + "' , tipo: '" + tipo + "'}"
        , beforeSend: function () { window.parent.showCargar(1); }
        , complete: function () { window.parent.showCargar(0); }
...
function GetFileData(codigo, fk_log_cargue, fk_Documento) {
    ...
    $.ajax({
        ...
        , data: "{codigo:'" + codigo + "', tipo:1, fk_log_Cargue: '" + fk_log_cargue + "', fk_Documento: '" + fk_Documento + "', llaves: '" + hdnllaves + "'}"
        , beforeSend: function () { window.parent.showCargar(1); }
        , complete: function () { window.parent.showCargar(0); }
...
function getHistorialEstados(codigo, fk_log_cargue, fk_Documento) {
    var hdnllaves = $('#hdnllavesFolder').val();
    ...
    $.ajax({
        ...
        , type: "POST"
        , url: Pagina + "GetFileData"
        , data: "{codigo:'" + codigo + "', tipo:3, fk_log_Cargue: '" + fk_log_cargue + "', fk_Documento: '" + fk_Documento + "', llaves: '" + hdnllaves + "'}"
        , beforeSend: function () { window.parent.showCargar(1); }
        , complete: function () { window.parent.showCargar(0); }
...
function verPdf(token) {

    var url = $("#PDFURL").val() + "?token=" + token;
    var params = ['height=' + screen.height, 'width=' + screen.width, 'fullscreen=yes'].join(',');
    //window.open(url, "visor", "height=650px,width=980px,top=10px");
